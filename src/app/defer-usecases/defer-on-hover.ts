...
@Component({
  ...
  template: `
    <app-box>
      <content>
        📚 <code>hover</code> triggers deferred loading when the mouse has
        hovered over the trigger area. Events used for this are
        <code>mouseenter</code> and <code>focusin</code>.
      </content>
    </app-box>

    @defer (on hover) {
    <lazy-on-hover>
      <article>some <code>&#64;defer (on hover)</code> content</article>
    </lazy-on-hover>
    <lazy-shared>
      <article>some <code>&#64;defer (on hover)</code> content</article>
    </lazy-shared>
    ...
    <app-button
      >👉 <strong>hover me</strong> (the placeholder) to load content
      👈</app-button
    >
    ...
    <app-button #el
      >👉 <strong>hover this guy</strong> (element with a template variable) to
      load content 👈</app-button
    >
    @defer (on hover(el)) {
    <lazy-on-hover>
      <article>some <code>&#64;defer (on hover(el))</code> content</article>
    </lazy-on-hover>
    <lazy-shared>
      <article>some <code>&#64;defer (on hover(el))</code> content</article>
    </lazy-shared>
