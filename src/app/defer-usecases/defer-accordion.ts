...
@Component({
  ...
  imports: [
    BoxComponent,
    LazyAccordionContentAComponent,
    LazyAccordionContentBComponent,
    LazyAccordionContentCComponent,
  ],
  styles: `
  .accordion {
  ...
  template: `
    <app-box>
      <content>
        🖐️ each accordion section uses:
        <code>&#64;defer (on hover(accordion); on timer(5s))</code>:
        <ul>
          <li>
            when <strong>two (or more) different triggers</strong> are declared,
            the first one that takes place triggers actual loading
          </li>
          <li>
            accordion section could start loading on
            <strong>interaction</strong> (when clicked), but loading on
            <strong>hover</strong> is usually better, since loading will start
    ...
    <section class="accordion-content">
      <button #accordionA class="accordion" (click)="toggleVisible(0)">
        Section A
      </button>
      <div class="panel">
      ...
      <button #accordionB class="accordion" (click)="toggleVisible(1)">
        Section B
      </button>
      <div class="panel">
      ...
      <button #accordionC class="accordion" (click)="toggleVisible(2)">
        Section C
      </button>
      <div class="panel">
